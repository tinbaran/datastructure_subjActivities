...
private int countNodes(BST r)
{
   ...
   {
   	int p = 1;
      p += countNodes(r.getLeft());
      p += countNodes(r.getRight());
      return p;
   }
}
...
private int smallest(BST r)
{
	if (r.left == null)
   	return Integer.parseInt(r.item.toString());
...
private int sum(BST r){
   int sum = 0;
   int leftSum = 0;
   int rightSum = 0;
   
   if(!isEmpty())
      sum = Integer.parseInt(r.item.toString());
   if(r.left != null)
      leftSum = sum(r.left);
   if(r.right != null)
